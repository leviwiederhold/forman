"use client";

import { useMemo, useState } from "react";
import Link from "next/link";
import { Input } from "@/components/ui/input";
import { cn } from "@/lib/utils";
import { Button } from "@/components/ui/button";

type QuoteRow = {
  id: string;
  trade: string | null;
  created_at: string;
  customer_name: string | null;
  status: string | null;
  total: number | null;
};

function fmtMoney(n: number | null) {
  if (n == null) return "$0.00";
  return new Intl.NumberFormat("en-US", {
    style: "currency",
    currency: "USD",
  }).format(n);
}

function statusMeta(status: string | null | undefined) {
  const s = (status ?? "draft").toLowerCase();

  if (s === "accepted") {
    return {
      label: "Accepted",
      className: "bg-[#e1f5e6] text-[#154625] border-[#154625]",
    };
  }

  if (s === "rejected") {
    return {
      label: "Rejected",
      className: "bg-[#ffdad6] text-destructive border-destructive",
    };
  }

  if (s === "sent") {
    return {
      label: "Sent",
      className: "bg-[#e6f1f6] text-[#004e6a] border-[#004e6a]",
    };
  }

  return {
    label: "Draft",
    className: "bg-muted text-foreground/80 border-border",
  };
}

export default function QuotesListClient({
  rows = [],
}: {
  rows?: QuoteRow[];
}) {
  const [query, setQuery] = useState("");
  const [items, setItems] = useState<QuoteRow[]>(rows);
  const [duplicatingId, setDuplicatingId] = useState<string | null>(null);

  async function deleteQuote(id: string) {
    const ok = window.confirm("Delete this quote? This cannot be undone.");
    if (!ok) return;

    // Optimistic remove
    const prev = items;
    setItems((cur) => cur.filter((q) => q.id !== id));

    try {
      const res = await fetch(`/api/quotes/${id}`, { method: "DELETE" });
      if (!res.ok) {
        const j = await res.json().catch(() => ({}));
        alert(j?.error ?? "Delete failed.");
        setItems(prev);
      }
    } catch {
      alert("Delete failed.");
      setItems(prev);
    }
  }

  async function duplicateQuote(id: string) {
    if (duplicatingId) return;
    setDuplicatingId(id);

    try {
      const res = await fetch(`/api/quotes/${id}/duplicate`, { method: "POST" });
      const json = (await res.json().catch(() => ({}))) as {
        id?: string;
        error?: string;
      };

      if (!res.ok || !json.id) {
        alert(json.error ?? "Duplicate failed.");
        return;
      }

      window.location.href = `/quotes/${json.id}/edit`;
    } finally {
      setDuplicatingId(null);
    }
  }

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return items;

    return items.filter((r) =>
      (r.customer_name ?? "").toLowerCase().includes(q)
    );
  }, [items, query]);

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4">
        <Input
          placeholder="Search customer names..."
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          className="max-w-sm"
        />
        <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-muted-foreground">
          {filtered.length} results
        </div>
      </div>

      <div className="paper-panel divide-y divide-[#dfbfbc]">
        {filtered.map((row) => {
          const meta = statusMeta(row.status);
          const total = row.total ?? 0;

          return (
            <div
              key={row.id}
              className="flex flex-col justify-between gap-4 px-4 py-5 transition-colors hover:bg-muted lg:flex-row lg:items-center"
            >
                <Link href={`/quotes/${row.id}`} className="min-w-0 flex-1">
                  <div className="grid gap-3 lg:grid-cols-[110px_minmax(220px,320px)_1fr] lg:items-start">
                    <div>
                      <div className="forman-kicker">ID</div>
                      <div className="mt-1 font-headline text-xl font-bold">#{row.id.slice(0, 8).toUpperCase()}</div>
                    </div>
                    <div className="space-y-1">
                      <div className="forman-kicker">Customer</div>
                      <div className="font-headline text-xl font-bold uppercase tracking-[-0.03em]">
                      {row.customer_name || "Unnamed customer"}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        {new Date(row.created_at).toLocaleDateString()}
                      </div>
                    </div>
                    <div className="space-y-1">
                      <div className="forman-kicker">Work order</div>
                      <div className="text-sm text-foreground/80">
                        {row.trade ?? "Roofing"} quote in {row.status ?? "draft"} status.
                      </div>
                    </div>
                  </div>
                </Link>

                <div className="flex flex-wrap items-center gap-3 lg:justify-end">
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                    disabled={Boolean(duplicatingId)}
                    onClick={() => duplicateQuote(row.id)}
                  >
                    {duplicatingId === row.id ? "Duplicating..." : "Duplicate"}
                  </Button>
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                    onClick={() => deleteQuote(row.id)}
                  >
                    Delete
                  </Button>

                  <div className="ml-auto flex min-w-[140px] flex-col items-end gap-2">
                  <span
                    className={cn(
                      "inline-flex items-center rounded-sm border px-2 py-1 text-[11px] font-bold uppercase tracking-[0.12em]",
                      meta.className
                    )}
                  >
                    {meta.label}
                  </span>
                  <div className="font-headline text-xl font-bold text-foreground/85">
                    {fmtMoney(total)}
                  </div>
                  </div>
                </div>
            </div>
          );
        })}

        {filtered.length === 0 && (
          <div className="px-4 py-10">
            {items.length === 0 && query.trim().length === 0 ? (
              <div className="mx-auto max-w-xl space-y-4 text-center">
                <div className="font-headline text-2xl font-bold uppercase">No quotes yet</div>
                <div className="text-sm text-muted-foreground">
                  Create your first quote in minutes. Set pricing once, send the quote, and track views and follow-up from your dashboard.
                </div>
                <div className="grid gap-2 text-left text-sm text-foreground/75">
                  <div className="paper-inset p-3">1. Confirm your pricing and margin targets.</div>
                  <div className="paper-inset p-3">2. Turn on deposits to reduce late cancellations.</div>
                  <div className="paper-inset p-3">3. Create and send your first quote.</div>
                </div>
                <div className="flex flex-wrap justify-center gap-2">
                  <Button asChild size="sm">
                    <Link href="/quotes/new">Create first quote</Link>
                  </Button>
                  <Button asChild variant="outline" size="sm">
                    <Link href="/settings/roofing">Pricing</Link>
                  </Button>
                  <Button asChild variant="outline" size="sm">
                    <Link href="/settings/billing">Deposits</Link>
                  </Button>
                </div>
              </div>
            ) : (
              <div className="text-center text-sm text-muted-foreground">No quotes found</div>
            )}
          </div>
        )}
      </div>
    </div>
  );
}
